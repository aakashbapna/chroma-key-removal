...
for name in ['makeup','watch']:
    im=Image.open(ROOT/'datasets/real_banners'/f'{name}.jpg').convert('RGB');x=np.asarray(im,np.float32)/255;h,w=x.shape[:2];alpha=model.alpha(x);raw=Image.fromarray(rgba(x,alpha));info={'route':'model_only_alpha'}
    ...
    raw.save(OUT/f'{name}_neural.png');deep.save(OUT/f'{name}_deeplab.png');images=[im,checker(raw),checker(deep)];ph=min(480,round(h/w*350));panel=Image.new('RGB',(1080,ph+40),'white');d=ImageDraw.Draw(panel)
    for k,(image,label) in enumerate(zip(images,['Input','Model-only alpha','DeepLab soft'])):
...
sheet=Image.new('RGB',(1080,sum(p.height for p in panels)),'white');top=0
