...
runs=[name for name in ['banner_run1','banner_run2'] if (ROOT/'reports/banner_runs'/name/'metrics.json').exists()];reports={name:json.loads((ROOT/'reports/banner_runs'/name/'training.json').read_text()) for name in runs}
...
model=tf.keras.models.load_model(ROOT/'models/v3/best.keras',compile=False);model.save(ROOT/'models/v3/resume.keras')
